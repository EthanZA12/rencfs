...
impl EncryptedFs {
    ...
    pub async fn release(&self, handle: u64) -> FsResult<()> {
        ...
        if let Some(ctx) = ctx {
            ...
            file.sync_all()?;
            fs_util::sync_dir(self.contents_path(ctx.ino).parent().unwrap())?;
    ...
    pub async fn set_len(&self, ino: u64, size: u64) -> FsResult<()> {
        ...
        fs_util::sync_dir(file_path.parent().unwrap())?;
...
fn read_or_create_key(
    ...
    let salt = if salt_path.exists() {
        ...
        file.flush()?;
        file.sync_all()?;
        fs_util::sync_dir(salt_path.parent().expect("oops, we don't have a parent"))?;
        salt
    };
    ...
    if key_path.exists() {
        ...
        let file = writer.finish()?;
        file.sync_all()?;
        fs_util::sync_dir(key_path.parent().unwrap())?;
        Ok(SecretBox::new(Box::new(key)))
    }
}
